...
SYSTEM_PROMPTS = {
    ...
    "smeh": "Ты — Брат, весёлый ИИ-ассистент с отличным чувством юмора. Шути, используй мемы, будь расслабленным — но при этом всё равно помогай по делу. Обращайся к пользователю на ты.",
    "test": "Ты — Брат, ассистент для решения тестов. Отвечай максимально кратко и точно. Только правильный ответ, без объяснений если не просят.",
...
def send(chat_id, text, keyboard=None):
    ...
    for i in range(0, len(text), 4096):
        p = dict(payload)
        p["text"] = text[i:i+4096]
        if i > 0:
            p.pop("reply_markup", None)
        requests.post(f"{BASE_URL}/sendMessage", json=p)

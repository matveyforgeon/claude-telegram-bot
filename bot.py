...
BANNED_USERS = set(
int(x.strip()) for x in os.environ.get("BANNED_USERS", "").split(",") if x.strip().isdigi
)
...
json.dump({str(k): v for k, v in user_data.items()}, f, ensure_ascii=False, inden
...
# ===== ПРОМПТЫ =====
SYSTEM_PROMPTS = {
"default": "Ты - универсальный ИИ-ассистент. Отвечай нейтрально, чётко и по делу. Без лиш
"delo": "Ты - строгий деловой ИИ-ассистент. Отвечай максимально чётко и кратко, только фа
"friend": "Ты - друг и весёлый ИИ-ассистент с отличным чувством юмора. Немного шути, будь
"test": "Ты - ассистент для решения тестов. Отвечай максимально кратко и точно. Только пр
...
"text_work": "Ты - профессиональный редактор текстов. Жди текст от пользователя и выполня
"skill_gpt": """You are a prompt director for GPT Image 2.0. Convert user concepts into p
Three formats:
...
if i == 0 and keyboard:
payload["reply_markup"] = {"keyboard": keyboard, "one_time_keyboard": True, "resi
...
r = requests.post(
FREETHEAI_URL,
headers={"Authorization": f"Bearer {FREETHEAI_KEY}", "Content-Type": "application
json=payload,
timeout=120
)
data = r.json()
...
Не удалось сгенерировать фото. Попробуй другой промпт или модель.")
...
name = u.get("name", "")
sub = " " if u.get("has_sub") else f"msg:{u.get('msg_count',0)}/{FREE_MSG_LIMIT}
...
ref_u["referral_bonus_msg"] = ref_u.get("referral_bonus_msg", 0) + REFERRAL_B
ref_u["referral_bonus_photo"] = ref_u.get("referral_bonus_photo", 0) + REFERR
ref_u["referrals"].append(chat_id)
u["referred_by"] = ref_id
save_db()
send(ref_id,
f" Друг зарегистрировался по твоей ссылке!\n"
f"+{REFERRAL_BONUS_MSG} сообщений и +{REFERRAL_BONUS_PHOTO} фото начислен
)
except: pass
# Админ команды
...
send(chat_id, " return
У тебя закончились бесплатные сообщения. Оформи подписку send_typing(chat_id)
send(chat_id, " return
Фото получено! Напиши что с ним сделать — опишу, переведу текст с ф
if photos and caption and u.get("mode") != "photo":
# Фото с подписью в обычном режиме — отвечаем на вопрос по фото через Claude
if not can_msg(chat_id):
send(chat_id, " У тебя закончились бесплатные сообщения. Оформи подписку return
send_typing(chat_id)
try:
через
...
send(chat_id,
f"Привет, {name}! \n"
f"Я - твой личный AI-ассистент. Помогу с задачами, текстами, идеями и ответами на
f" Режим: Дефолт\n"
f" Модель: {model_name}\n\n"
f" Команды:\n"
...
return
if text == "/model":
current = CLAUDE_MODEL_NAMES.get(u.get("model", "sonnet"), " send(chat_id, f"Текущая модель: {current}\n\nВыбери:", [[" return
Sonnet 4.6")
Sonnet 4.6", " Opus 4
if text == "/lang":
return
...
send(chat_id, "Ты уверен? Вся история будет удалена.", [[" return
Точно очистить", " Нет
if text == "/sub":
if u.get("has_sub"):
...
f"За каждого друга: +{REFERRAL_BONUS_MSG} сообщений и +{REFERRAL_BONUS_PHOTO} фот
f"Твоя ссылка:\n{ref_link}\n\n"
...
data={"chat_id": chat_id, "caption": f"База пользователей — {len(user_dat
...
"test": " Тесты", "english": " Английский", "text_work": " Работа с "photo": " Фотографии", "skill_gpt": " GPT Image", "skill_sales": " текстом
AI Sal
}
send(chat_id,
f" *Твой статус*\n\n"
...
u["model"] = "opus"
save_db()
send(chat_id, "Модель: return
Opus 4.7 доступен только по подписке. Пиши @staremenow")
Opus 4.7 - максимальный интеллект.")
# Язык
if text == " Русский":
u["language"] = "ru"; save_db(); send(chat_id, "Язык: if text == " English":
u["language"] = "en"; save_db(); send(chat_id, "Language: if text == " Авто":
u["language"] = "auto"; save_db(); send(chat_id, "Язык: Русский"); return
English"); return
Авто"); return
...
show_photo_model_menu(chat_id)
Закончились бесплатные фото. /sub или /ref")
else:
...
if not can_msg(chat_id):
send(chat_id, " return
Закончились бесплатные сообщения.\n\nОформи /sub или пригласи друга
